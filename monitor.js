...
//
var util = require("./src/logger").getLogger();
var fs = require("fs");
...
function run (args){
  ...
  if (!program) {
	  program = module.parent.filename
  }
  if (!watch) {
    watch = ".";
  }
  ...
  pidfile = pidfile || process.cwd()+'/pid.supervisor'; 
...
function watchGivenFile (watch, poll_interval) {
  if (isWindows)
    fs.watch(watch, { persistent: true, interval: poll_interval },function(){

		log.info("file change:",watch," restart")
		if(isWindows)
			crashWin()
		else{
			crashOther()
